...
import {
  ...
  CheckCircle, EmojiEvents, ArrowBack,
...
const Certificates: React.FC = () => {
  ...
  const loadCertificates = async () => {
    try {
      ...
      const mappedCerts = data.map((cert: any) => ({
        ...
        issuedDate: cert.issuedAt || cert.issuedDate || new Date().toISOString(),
  ...
  const handleDownload = async (certificateId: number) => {
    try {
      ...
      alert('Certificate PDF download will be available soon. You can print this page for now.');
    } catch (error) {
                ...
                >
                  <Avatar
                    sx={{
                      width: 60,
                      height: 60,
                      bgcolor: certificate.verified ? 'success.main' : 'grey.400',
                      boxShadow: 3,
                    }}
                  >
                ...
                <CardContent sx={{ pt: 4 }}>
                  <Box sx={{ mb: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                      <Chip
                        label={certificate.verified ? 'VERIFIED' : 'PENDING'}
                        size="small"
                        color={certificate.verified ? 'success' : 'default'}
                      />
                    </Box>
                  </Box>
