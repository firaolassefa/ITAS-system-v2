...
const StaffDashboard: React.FC<{ user?: any }> = ({ user }) => {
  ...
  useEffect(() => {
    ...
    if (currentUser?.id) loadDashboard(currentUser);
  }, [user]);
  ...
  const stats = [
    { label: 'Training Courses', value: data.enrolledCourses, icon: <School />, color: BLUE },
    { label: 'Completed', value: data.completedCourses, icon: <CheckCircle />, color: GOLD },
    { label: 'Compliance', value: `${data.complianceScore}%`, icon: <Security />, color: BLUE },
    { label: 'Progress', value: `${data.averageProgress}%`, icon: <TrendingUp />, color: GOLD },
  ];
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box>
          ...
          <Chip label="MOR Staff" sx={{ fontWeight: 600, bgcolor: BLUE, color: 'white' }} />
        </Box>
        <Avatar sx={{ width: 64, height: 64, bgcolor: BLUE, fontSize: '1.5rem', fontWeight: 700 }}>
      ...
      {data.activeCourses.length > 0 && (
        <Box sx={{ mb: 4 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 3, gap: 1.5 }}>
            ...
            <Typography variant="h5" sx={{ fontWeight: 700 }}>Internal Training</Typography>
          </Box>
          <Grid container spacing={3}>
            {data.activeCourses.slice(0, 3).map((course: any, index: number) => (
              <Grid item xs={12} md={4} key={course.id || index}>
                <Card elevation={0} sx={{ border: '1px solid #e0e0e0', borderRadius: 2, transition: 'all 0.3s', '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 8px 24px rgba(0,0,0,0.1)' } }}>
                  <CardContent sx={{ p: 3 }}>
                    ...
                    <Box sx={{ mb: 2 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                        ...
                        <Typography variant="body2" sx={{ fontWeight: 600 }}>{course.progress || 0}%</Typography>
                      </Box>
                      <LinearProgress
                        variant="determinate"
                        value={course.progress || 0}
                        sx={{ height: 8, borderRadius: 4, bgcolor: '#e0e0e0', '& .MuiLinearProgress-bar': { bgcolor: BLUE, borderRadius: 4 } }}
                      />
                    </Box>
                    <Button
                      fullWidth
                      ...
                      sx={{ bgcolor: BLUE, textTransform: 'none', fontWeight: 600, '&:hover': { bgcolor: '#1c7ed6' } }}
                    >
                      Continue
                    </Button>
                  </CardContent>
